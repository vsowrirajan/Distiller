...
public class Record {
	...
	public void setTimestamp(long timestamp) throws Exception {
		if (this.previousTimestamp > timestamp)
			throw new Exception("Invalid value for setTimestamp, timestamp:"
					+ timestamp + " previousTimestamp:" + previousTimestamp);
		this.timestamp = timestamp;
	}
	...
	public void setPreviousTimestamp(long previousTimestamp) throws Exception {
		if (this.timestamp < previousTimestamp)
			throw new Exception(
					"Invalid value for setPreviousTimestamp, timestamp:"
							+ this.timestamp + " previousTimestamp:"
							+ previousTimestamp);
		this.previousTimestamp = previousTimestamp;
	}
	...
	public String toString() {
		if (previousTimestamp == -1 && timestamp != -1)
			...
			return "T:" + timestamp + " P:" + previousTimestamp + " D:"
					+ getDurationms();
		return "";
	}
}
