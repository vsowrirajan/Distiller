...
public class ThreadResourceRecordProcessor implements RecordProcessor<Record> {
	...
	private static final Logger LOG = LoggerFactory
			.getLogger(ThreadResourceRecordProcessor.class);
	...
	public DifferentialValueRecord diff(Record rec1, Record rec2, String metric)
		...
		if (rec1.getPreviousTimestamp() == -1l
			...
			throw new Exception(
					"ThreadResourceRecords can only be diff'd from non-raw ThreadResourceRecords");
		...
		if (oldRecord.get_starttime() != newRecord.get_starttime()
			...
			throw new Exception(
					"Can not calculate diff for input records that are from different threads");
		...
		case "readCallRate":
			return new DifferentialValueRecord(
					oldRecord.getPreviousTimestamp(), oldRecord.getTimestamp(),
					newRecord.getPreviousTimestamp(), newRecord.getTimestamp(),
					getName(), metric, "double", newRecord.getReadCallRate()
							- oldRecord.getReadCallRate());

		case "writeCallRate":
			return new DifferentialValueRecord(
					oldRecord.getPreviousTimestamp(), oldRecord.getTimestamp(),
					newRecord.getPreviousTimestamp(), newRecord.getTimestamp(),
					getName(), metric, "double", newRecord.getWriteCallRate()
							- oldRecord.getWriteCallRate());

		case "readCharRate":
			return new DifferentialValueRecord(
					oldRecord.getPreviousTimestamp(), oldRecord.getTimestamp(),
					newRecord.getPreviousTimestamp(), newRecord.getTimestamp(),
					getName(), metric, "double", newRecord.getReadCharRate()
							- oldRecord.getReadCharRate());

		case "writeCharRate":
			return new DifferentialValueRecord(
					oldRecord.getPreviousTimestamp(), oldRecord.getTimestamp(),
					newRecord.getPreviousTimestamp(), newRecord.getTimestamp(),
					getName(), metric, "double", newRecord.getWriteCharRate()
							- oldRecord.getWriteCharRate());
		...
		case "cancelledWriteByteRate":
			return new DifferentialValueRecord(
					...
					newRecord.getCancelledWriteByteRate()
							- oldRecord.getCancelledWriteByteRate());
