...
public class ProcessResourceRecord extends Record {
	 ...
	 * DERIVED VALUES
	 */
	private double cpuUtilPct, iowaitUtilPct, readIoCallRate, writeIoCallRate,
			readIoCharRate, writeIoCharRate, readIoByteRate, writeIoByteRate,
			cancelledWriteIoByteRate;
	...
	private BigInteger cguest_time, cmajflt, cminflt, cstime, cutime,
			delayacct_blkio_ticks, guest_time, majflt, minflt, rss, rsslim,
			stime, utime, vsize;
	private BigInteger rchar, wchar, syscr, syscw, read_bytes, write_bytes,
			cancelled_write_bytes;

	/**
	 * CONSTRUCTORS
	 */
	public ProcessResourceRecord(ProcessResourceRecord rec1,
			ProcessResourceRecord rec2) throws Exception {
		if (rec1.getTimestamp() == rec2.getTimestamp())
			throw new Exception(
					"Can not generate differential ProcessResourceRecord from input records with matching timestamp values");
		if (rec1.get_starttime() != rec2.get_starttime()
				|| rec1.get_pid() != rec2.get_pid())
			throw new Exception(
					"Differential ProcessResourceRecord can only be generated from input records from the same process");
		...
		if (rec1.getCpuUtilPct() == -1d && rec2.getCpuUtilPct() == -1d) {
			...
			// Differential values:
			this.cguest_time = newRecord.get_cguest_time().subtract(
					oldRecord.get_cguest_time());
			this.cmajflt = newRecord.get_cmajflt().subtract(
					oldRecord.get_cmajflt());
			this.cminflt = newRecord.get_cminflt().subtract(
					oldRecord.get_cminflt());
			this.cstime = newRecord.get_cstime().subtract(
					oldRecord.get_cstime());
			this.cutime = newRecord.get_cutime().subtract(
					oldRecord.get_cutime());
			this.delayacct_blkio_ticks = newRecord.get_delayacct_blkio_ticks()
					.subtract(oldRecord.get_delayacct_blkio_ticks());
			this.guest_time = newRecord.get_guest_time().subtract(
					oldRecord.get_guest_time());
			this.majflt = newRecord.get_majflt().subtract(
					oldRecord.get_majflt());
			this.minflt = newRecord.get_minflt().subtract(
					oldRecord.get_minflt());
			...
			this.read_bytes = newRecord.get_read_bytes().subtract(
					oldRecord.get_read_bytes());
			this.write_bytes = newRecord.get_write_bytes().subtract(
					oldRecord.get_write_bytes());
			this.cancelled_write_bytes = newRecord.get_cancelled_write_bytes()
					.subtract(oldRecord.get_cancelled_write_bytes());

			// Derived values:
			this.rss = newRecord.get_rss().multiply(
					new BigInteger(Long.toString(this.getDurationms())));
			this.vsize = newRecord.get_vsize().multiply(
					new BigInteger(Long.toString(this.getDurationms())));
			...
			if (oldRecord.getTimestamp() != newRecord.getPreviousTimestamp())
				throw new Exception(
						"Can not generate differential ProcessResourceRecord from non-consecutive differential records");
			// Copied values:
			this.setTimestamp(newRecord.getTimestamp()); // Set the end
															// timestamp to the
															// timestamp of the
															// newer record
			this.setPreviousTimestamp(oldRecord.getPreviousTimestamp()); // Set
																			// the
																			// start
																			// timestamp
																			// to
			...
			// Differential values:
			this.cguest_time = newRecord.get_cguest_time().add(
					oldRecord.get_cguest_time());
			...
			this.delayacct_blkio_ticks = newRecord.get_delayacct_blkio_ticks()
					.add(oldRecord.get_delayacct_blkio_ticks());
			this.guest_time = newRecord.get_guest_time().add(
					oldRecord.get_guest_time());
			...
			this.read_bytes = newRecord.get_read_bytes().add(
					oldRecord.get_read_bytes());
			this.write_bytes = newRecord.get_write_bytes().add(
					oldRecord.get_write_bytes());
			this.cancelled_write_bytes = newRecord.get_cancelled_write_bytes()
					.add(oldRecord.get_cancelled_write_bytes());
			...
			throw new Exception(
					"Can not generate differential ProcessResourceRecord from a raw record and a differential record");
		}
		this.cpuUtilPct = this.utime.add(this.stime).doubleValue() / // The
																		// number
																		// of
																		// jiffies
																		// used
																		...
																		// the
																		// duration
				(((double) (this.clockTick * this.getDurationms())) / 1000d); // The
																				// number
																				// of
																				// jiffies
																				// that
																				...
																				// the
																				// duration
		this.iowaitUtilPct = this.delayacct_blkio_ticks.doubleValue() / // The
																		// number
																		// of
																		// jiffies
																		// the
		...
		this.writeIoByteRate = this.write_bytes.doubleValue()
				/ (((double) (this.clockTick * this.getDurationms())) / 1000d);
		this.cancelledWriteIoByteRate = this.cancelled_write_bytes
				.doubleValue()
				/ (((double) (this.clockTick * this.getDurationms())) / 1000d);
	}

	public ProcessResourceRecord(String statpath, String iopath, int clockTick)
			throws Exception {
		// This constructor takes "path" which should be the path to a
		// /proc/[pid]/stat file
		// This constructor takes "clockTick" which should be set to jiffies per
		// second from kernel build (obtained in ProcRecordProducer)
		...
		String[] parts = null;
		int statbs = 600; // 600 bytes should be enough to hold contents of
							// /proc/[pid]/stat or /proc/[pid]/task/[tid]/stat
		int iobs = 250; // 250 bytes should be enough to hold contents of
						// /proc/[pid]/io or /proc/[pid]/task/[tid]/io
		...
		try {
			...
			try {
				...
				throw new Exception(
						"Failed to produce a ProcessResourceRecord due to an exception reading the stats file: "
								+ statpath, e);
			}

			// Check whether we were able to read from the file, and whether we
			// were able to read the entire contents
			if (br > 0 && br < statbs) {
				...
				// Expect 44 values in /proc/[pid]/stat based on Linux kernel
				// version used for this dev.
				// Note that 42 is used in below check because
				if (parts.length < 42) {
					throw new Exception(
							"Failed to produce a ProcessResourceRecord due to unexpected format of stat file, found "
									+ parts.length + " fields");
				}
				...
				try {
					...
					throw new Exception(
							"Failed to produce a ProcessResourceRecord due to an exception reading the io file: "
									+ iopath, e);
				}
				String line = null;
				if (br > 0 && br < iobs) {
					...
					throw new Exception(
							"Failed to produce a ProcessResourceRecord from io file due to read response length, br:"
									+ br + " bs:" + iobs);
				}
				parts = line.trim().split("\\s+");
				if (parts.length < 14) { // Expect 14 values in
											// /proc/[pid]/task/[tid]/io based
											// on Linux kernel version used for
											// this dev.
					throw new Exception(
							"Failed to produce a ProcessResourceRecord due to unexpected format of io file, found "
									+ parts.length + " fields");
				}
				...
				throw new Exception(
						"Failed to produce a ProcessResourceRecord due to read response length, br:"
								+ br + " bs:" + statbs);
			}
		} catch (Exception e) {
	...
	public static int[] produceRecord(RecordQueue outputQueue,
			String producerName, String statpath, String iopath, int clockTick) {
		...
		if (record != null && !outputQueue.put(producerName, record)) {
			ret[3] = 1;
			System.err
					.println("Failed to put ProcessResourceRecord into output queue "
	...
	public String toString() {
		return super.toString() + " process.resources: " + pid + " " + comm
				+ " " + state + " " + ppid + " " + num_threads;
	}
