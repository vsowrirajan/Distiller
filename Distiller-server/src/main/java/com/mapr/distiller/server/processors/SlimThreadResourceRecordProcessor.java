...
public class SlimThreadResourceRecordProcessor implements
	...
	public DifferentialValueRecord diff(Record rec1, Record rec2, String metric)
		...
		if (rec1.getPreviousTimestamp() == -1l
			...
			throw new Exception(
					"SlimThreadResourceRecords can only be diff'd from non-raw SlimThreadResourceRecords");
	...
	public boolean isEqual(Record record, String metric, String thresholdValue)
		...
		default:
			throw new Exception("Metric " + metric
					+ " is not Thresholdable in SlimThreadResourceRecord for "
	...
	public boolean isAboveThreshold(Record record, String metric,
		...
		case "idlePct":
			if (currentRecord.getCpuUtilPct() == -1d
					|| currentRecord.getIowaitUtilPct() == -1d)
				throw new Exception(
						"Can not compare raw SlimThreadResourceRecord to threshold");
			else
				return 1d - currentRecord.getCpuUtilPct()
						- currentRecord.getIowaitUtilPct() > Double
							.parseDouble(thresholdValue);
