...
public class RawRecordProducerStatusRecord extends Record {
	...
	private String producerId; // This should uniquely identify what generated
								...
								// ProcRecordProducer)
	private long recordsCreated, // The number of Records created by the
									// producer
			recordCreationFailures, // The number of times the producer tried to
									// create a Record and failed
			queuePutFailures, // The number of calls to put a Record into a
								// RecordQueue that did not return successfully
			...
			runningTimems; // The amount of time the record producer was
							...
							// producer
	private HashMap<String, String> extraInfo = null; // Extra info that a
														// record producer can
														// choose to include.
	...
	public RawRecordProducerStatusRecord(RawRecordProducerStatusRecord oldRecord)
			throws Exception {
		super(System.currentTimeMillis());
		...
		try {
			...
			throw new Exception("Failed to set timestamps on old record: "
		...
		if (this.getPreviousTimestamp() != -1l)
			cpuString = "\tCPU:"
					+ (((double) runningTimems)
							/ ((double) this.getDurationms()) * 100d) + "%";
		String eiString = "";
		if (extraInfo != null) {
			Iterator<Map.Entry<String, String>> i = extraInfo.entrySet()
					.iterator();
			while (i.hasNext()) {
				Map.Entry<String, String> p = (Map.Entry<String, String>) i
						.next();
				// eiString=eiString + "\t" + p.getKey() + ":" + p.getValue();
				if (!p.getValue().equals("0"))
					eiString = eiString + "\t" + p.getKey() + ":"
							+ p.getValue();
			}
		}
		return super.toString() + ":RPSRec:" + producerId + "\truntime:"
				+ runningTimems + "\trecC:" + recordsCreated + "\trFail:"
				+ recordCreationFailures + "\tpFail:" + queuePutFailures
				+ "\toFail:" + otherFailures + cpuString + eiString;
	}
