...
public class BasicRelatedRecordSelector implements
		RelatedRecordSelector<Record, Record> {
	...
	public BasicRelatedRecordSelector(String id, // The string to be used when
													// calling get/put against
													// queues
			RecordQueue relatedQueue, // e.g. MfsGuts-1s-Raw
			RecordQueue outputQueue, // e.g.
										// MfsGUts-1s-Raw-During-MfsThread-HighCpu
	...
	public long[] selectRelatedRecords(Record inputRecord) throws Exception {
		...
		if (relationMethod.equals(Constants.TIME_BASED_WINDOW)) {
			// Update the list of TimePeriods we are interested in based on the
			// timestamps in the new input record
			if (relationKey.equals("duration")) {
				TimePeriod p = new TimePeriod();
				if (relatedTimePeriods.size() != 0
						&& inputRecord.getPreviousTimestamp() <= relatedTimePeriods
								.getLast().end) {
					if (inputRecord.getTimestamp() < relatedTimePeriods
							.getLast().start)
						throw new Exception(
								"End timestamp of current input record is less than start timestamp of previous input record, records must be in chronological order.");
					...
					if (inputRecord.getPreviousTimestamp() == -1)
						throw new Exception(
								"Can not perform duration based window selection for raw input records.");
					p.start = inputRecord.getPreviousTimestamp() - window;
					p.end = inputRecord.getTimestamp() + window;
				...
				throw new Exception("Unknown key " + relationKey
						+ " for method " + Constants.TIME_BASED_WINDOW);
			}
			// Check the related queue against the new list of related
			// TimePeriods to see if there are matching records to output
			relatedRecord = relatedQueue.peek(id, false);

			while (relatedRecord != null) {
				if (relatedRecord.getPreviousTimestamp() != -1
						&& relatedRecord.getPreviousTimestamp() > relatedTimePeriods
								.getLast().end) {
					// The start timestamp of the oldest record in the related
					// queue is newer than the end timestamp of the newest
					// related TimePeriod, leave the Records in the related
					// queue.
					break;
				...
				} else if (relatedRecord.getTimestamp() > relatedTimePeriods
						.getLast().end) {
					break;
				} else if (relatedRecord.getTimestamp() <= relatedTimePeriods
						.getLast().end
						&& relatedRecord.getTimestamp() >= relatedTimePeriods
								.getFirst().start) {
					boolean matchesWindow = false;
					Iterator<TimePeriod> i = relatedTimePeriods.iterator();
					while (i.hasNext()) {
		...
		return new long[] { inRecCntr, outRecCntr, putFailureCntr,
				otherFailureCntr };
	}
}
