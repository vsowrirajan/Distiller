...
public class Schedule {
	...
	public String toString() {
		return "lsst:" + lastScheduledStartTime + " lst:" + lastStartTime
				+ " let:" + lastEndTime + " per:" + periodicity + " mdp:"
				+ maxDurationPct + " mwp:" + minWaitPct + " delay:"
				+ (lastStartTime + periodicity - lastScheduledStartTime);
	}
	...
	public Schedule(long periodicity, double maxDurationPct, double minWaitPct)
		...
		if (periodicity < 1000)
			throw new Exception(
					"Minimum scheduling periodicity is 1000 (ms), value provided: "
							+ periodicity);
		if (maxDurationPct < 0d | maxDurationPct > 1d)
			throw new Exception("Invalid maxDurationPct: " + maxDurationPct
					+ ", expected 0 <= maxDurationPct <= 1");
		if (minWaitPct < 0d | minWaitPct > 1d)
			throw new Exception("Invalid maxDurationPct: " + maxDurationPct
					+ ", expected 0 <= maxDurationPct <= 1");
		if (maxDurationPct + minWaitPct > 1d)
			throw new Exception(
					"Combined value of maxDurationPct and minWaitPct must be <= 1, minWaitPct: "
							+ minWaitPct + " maxDurationPct: " + maxDurationPct);
	...
	public void advanceSchedule() throws Exception {
		...
		if ((lastStartTime != -1 || lastEndTime != -1)
				&& (lastStartTime < lastScheduledStartTime || lastEndTime < lastStartTime))
			throw new Exception("Invalid timestamps.  lastScheduledStartTime:"
					+ lastScheduledStartTime + " lastStartTime:"
		...
		long waitTimeForIdealStartTime = idealNextScheduledStartTime
				- System.currentTimeMillis();
		double waitTimePct = (double) waitTimeForIdealStartTime
				/ (double) periodicity;
		...
		if (durationPct > maxDurationPct) {
			...
			if (lastScheduledStartTime + adjustedPeriodicity < lastEndTime
					+ (long) ((double) adjustedPeriodicity * minWaitPct)) {
				lastScheduledStartTime = lastEndTime
						+ (long) ((double) adjustedPeriodicity * minWaitPct);
			} else {
			...
			if (waitTimePct < minWaitPct) {
				lastScheduledStartTime = lastEndTime
						+ ((long) (minWaitPct * (double) periodicity));
			} else {
