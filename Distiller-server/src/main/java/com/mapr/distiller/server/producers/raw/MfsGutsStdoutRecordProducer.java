...
public class MfsGutsStdoutRecordProducer extends Thread {
	...
	public MfsGutsStdoutRecordProducer(InputStream stdout,
			RecordQueue outputQueue, RecordQueue producerStatsQueue,
			String producerName) {
		...
		while (!shouldExit) {
			...
			if (((System.currentTimeMillis() - mystatus.getTimestamp()) / 1000l) >= statusIntervalSeconds) {
				...
				if (producerMetricsEnabled
					...
					LOG.error(
							"Failed to put RecordProducerStatusRecord to output queue {}  size: {}  maxSize: {}  producerName: {}",
							producerStatsQueue.getQueueName(),
							producerStatsQueue.queueSize(),
							producerStatsQueue.getQueueRecordCapacity(),
							producerName);
				}
				mystatus = newRecord;
			}
			...
			if (line != null) {
				if (isHeader(line)) {
					// This is all a hack... hard coded header and all. See MapR
					// bug 19735.
					if (!verifiedHeader) {
					...
					if (verifiedHeader) {
						...
						// Somehow, we have a sample line but no header line was
						// found.
						// We can't parse a sample line without knowing the
						// header.
						break;
					}
				}
				// else skip it, because guts prints blank lines for whatever
				// reason...
			} else {
				LOG.error("ERROR: Could not read from MFS guts stdout.");
				break;
			...
			mystatus.setRunningTimems(mystatus.getRunningTimems()
