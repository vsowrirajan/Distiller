...
public class MfsGutsRecordProducer extends Thread {
	...
	public boolean producerMetricsEnabled() {
		return (mfsGutsStdoutRecordProducer != null && mfsGutsStdoutRecordProducer
	...
	public boolean enableProducerMetrics(RecordQueue producerStatsQueue) {
		if (producerStatsQueue != null) {
			...
			if (mfsGutsStdoutRecordProducer != null) {
				return mfsGutsStdoutRecordProducer
						.enableProducerMetrics(producerStatsQueue);
			}
			return true;
		}
		return false;
	...
	private MfsId readMfsIdFromFile(String path) throws Exception {
		...
		try {
			...
			if (line.split("\\(", 2)[1].split("\\)", 2)[0].equals("mfs")) {
				...
				mfsId.starttime = Integer.parseInt(line.split("\\)", 2)[1]
						.trim().split("\\s+")[19]);
				return mfsId;
			} else {
				throw new Exception("File " + path
						+ " is not a MFS process stat file");
			}
		} catch (Exception e) {
	...
	public void run() {
		...
		// E.g.:
		// /opt/mapr/bin/guts flush:line time:none cpu:none net:none disk:none
		// ssd:none cleaner:all fs:all kv:all btree:all rpc:db db:all dbrepl:all
		// cache:none log:all resync:all io:small
		ProcessBuilder mfsGutsProcessBuilder = new ProcessBuilder(
				"/opt/mapr/bin/guts", "flush:line", "time:none", "cpu:none",
				"net:none", "disk:none", "ssd:none", "cleaner:all", "fs:all",
				"kv:all", "btree:all", "rpc:db", "db:all", "dbrepl:all",
				"cache:none", "log:all", "resync:all", "io:small");

		while (!shouldExit) {
			// First, find the PID of MFS, both to make sure it's running (so
			// guts prints proper output) and
			// so we can monitor for termination of the process since we need to
			// restart guts when that happens
			try {
			...
			if (mfsPid != -1) {
				// We have the MFS PID, now check /proc/[pid]/stat to ensure the
				// command name is "(mfs)" and retrieve the start time
				MfsId mfsId = null;
				try {
					mfsId = readMfsIdFromFile("/proc/" + mfsPid + "/stat");
					mfsAlive = true;
				} catch (Exception e) {
					if (System.currentTimeMillis() > lastTimeMfsDownPrinted
							+ minTime) {
						LOG.info(
								"Found MFS pid {}  from pid file, but could not identify MFS process running from /proc/ {} /stat, will try again...",
								mfsPid, mfsPid);
				...
				if (mfsAlive && !shouldExit) {
					// If MFS is alive, then try to create an MFS guts
					// process...
					...
					if (gutsAlive) {
						// If MFS guts is alive, then start processing it's
						// output.
						mfsGutsStdoutRecordProducer = new MfsGutsStdoutRecordProducer(
								mfsGutsProcess.getInputStream(), outputQueue,
								producerStatsQueue, producerName);
						...
						while (true) {
							...
							try {
								if (!mfsId.equals(readMfsIdFromFile("/proc/"
										+ mfsPid + "/stat"))) {
									LOG.info(
											"MFS process is no longer running with PID {} and starttime {}",
											mfsId.pid, mfsId.starttime);
									break;
								}
							} catch (Exception e) {
								LOG.error(
										"Failed to determine if MFS process is still running with PID {} and starttime {}",
										mfsId.pid, mfsId.starttime);
								break;
							}
							// Check if MfsGutsStdoutRecordProducer is still
							// running
							if (!mfsGutsStdoutRecordProducer.isAlive()) {
								if (!shouldExit)
									LOG.error("MfsGutsStdoutRecordProducer is not running.");
								break;
							}
							...
							try {
								...
								LOG.error(
										"MFS guts process exited with return code {}",
										gutsRetCode);
								break;
							} catch (IllegalThreadStateException e) {
								// Guts is still running, do nothing...
							} catch (Exception e) {
								// For any other exception while checking guts,
						...
						// Clean up here, stop/destroy guts process and stdout
						// processor so they can be recreated next iteration.
						mfsGutsStdoutRecordProducer.requestExit();
						try {
		...
		if (mfsGutsStdoutRecordProducer != null) {
			LOG.info(
					"MfsGutsRecordProducer. {}  is waiting for MfsGutsStdoutRecordProducer to exit",
		...
		LOG.info("MfsGutsRecordProducer. {}  exited.",
				System.identityHashCode(this));
	}
}
