...
public class DefaultConfiguration {
	...
	public static Properties getConfiguration(Properties otherProperties,
			boolean applyOtherPropertiesFirst) {
	...
	public static Properties setDefaultProperties(Properties configuration) {
		...
		configuration.setProperty("monitor.network.connections.queuesizes",
				"true");
		configuration.setProperty("monitor.mfs.guts", "true");
		configuration.setProperty("distill.system.cpu.usage", "true");
		configuration.setProperty("distill.network.connections.queuesizes",
				"true");
	...
	public static Properties applyConfigurationFile(Properties configuration,
		...
		try {
			...
			LOG.error("Failed to read configuration from file: {} ",
					configurationFilePath);
			System.exit(1);
		}
		LOG.debug("Applied configuration from file: {} ",
				configurationFilePath);
		return configuration;
	}
}
