...
public class SystemMemoryRecordProcessor implements RecordProcessor<Record> {
	...
	public DifferentialValueRecord diff(Record rec1, Record rec2, String metric)
		...
		if (rec1.getPreviousTimestamp() == -1l
			...
			throw new Exception(
					"SystemMemoryRecords can only be diff'd from non-raw SystemMemoryRecords");
		...
		case "pswpin":
			return new DifferentialValueRecord(
					...
					getName(), metric, "BigInteger", newRecord.get_pswpin()
							.subtract(oldRecord.get_pswpin()));

		case "pswpout":
			return new DifferentialValueRecord(
					oldRecord.getPreviousTimestamp(), oldRecord.getTimestamp(),
					newRecord.getPreviousTimestamp(), newRecord.getTimestamp(),
					getName(), metric, "BigInteger", newRecord.get_pswpout()
							.subtract(oldRecord.get_pswpout()));

		case "allocstall":
			return new DifferentialValueRecord(
					oldRecord.getPreviousTimestamp(), oldRecord.getTimestamp(),
					newRecord.getPreviousTimestamp(), newRecord.getTimestamp(),
					getName(), metric, "BigInteger", newRecord.get_allocstall()
							.subtract(oldRecord.get_allocstall()));

		case "%free":
			return new DifferentialValueRecord(
					oldRecord.getPreviousTimestamp(), oldRecord.getTimestamp(),
					newRecord.getPreviousTimestamp(), newRecord.getTimestamp(),
					getName(), metric, "double", newRecord.getFreeMemPct()
							- oldRecord.getFreeMemPct());
	...
	public boolean isEqual(Record record, String metric, String thresholdValue)
		...
		case "pswpin":
			return currentRecord.get_pswpin().equals(
					new BigInteger(thresholdValue));

		case "pswpout":
			return currentRecord.get_pswpout().equals(
					new BigInteger(thresholdValue));

		case "allocstall":
			return currentRecord.get_allocstall().equals(
					new BigInteger(thresholdValue));
	...
	public boolean isAboveThreshold(Record record, String metric,
		...
		case "pswpin":
			return currentRecord.get_pswpin().compareTo(
					new BigInteger(thresholdValue)) == 1;

		case "pswpout":
			return currentRecord.get_pswpout().compareTo(
					new BigInteger(thresholdValue)) == 1;

		case "allocstall":
			return currentRecord.get_allocstall().compareTo(
					new BigInteger(thresholdValue)) == 1;
	...
	@Override
	public SystemMemoryRecord merge(Record rec1, Record rec2) throws Exception {
