...
public class TcpConnectionStatRecord extends Record {
	 ...
	 * CONSTRUCTORS
	 */
	public TcpConnectionStatRecord(TcpConnectionStatRecord rec1,
		...
		if (rec1.getTimestamp() == rec2.getTimestamp())
			throw new Exception(
					"Can not generate differential TcpConnectionStatRecord from input records with matching timestamp values");
		if (rec1.getLocalIp() != rec2.getLocalIp()
			...
			throw new Exception(
					"Can not generate differential TcpConnectionStatRecord for input records for different connections");
		...
		if (oldRecord.getPreviousTimestamp() != -1
			...
			// Don't accept an old record that is raw and a new that is
			// differential
			if (oldRecord.getPreviousTimestamp() == -1)
				throw new Exception(
						"Can not generate differential TcpConnectionStatRecord from an older raw record and a newer differential record.");
			// If they are both differential records
			if (oldRecord.getPreviousTimestamp() != -1
					&& newRecord.getPreviousTimestamp() != -1) {
				// Don't accept non-consecutive differential records as input
				if (oldRecord.getTimestamp() != newRecord
						.getPreviousTimestamp())
					throw new Exception(
							"Can not generate differential TcpConnectionStatRecord for non-consecutive differential input records");
				// We have consecutive, differential records as input. Add the
				// queue sizes
				this.rxQ = newRecord.get_rxQ().add(oldRecord.get_rxQ());
				this.txQ = newRecord.get_txQ().add(oldRecord.get_txQ());
			} else {
				// The older record is differential and newer record is raw,
				// calculate rxQ and txQ byte-seconds for elapsed time from new
				// record timetstamp to old record timestamp and add it to old
				// record value
				try {
					oldRecord.get_rxQ().add(new BigInteger("1"));
				} catch (Exception e) {
					LOG.error("Bad oldRecord rxQ, record: {}",
							oldRecord.toString());
				}
				oldRecord.get_rxQ().add(
						newRecord.get_rxQ().multiply(
								new BigInteger(Long.toString(newRecord
										.getTimestamp()
										- oldRecord.getTimestamp()))));
				this.rxQ = oldRecord.get_rxQ().add(
						newRecord.get_rxQ().multiply(
								new BigInteger(Long.toString(newRecord
										.getTimestamp()
										- oldRecord.getTimestamp()))));
				this.txQ = oldRecord.get_txQ().add(
						newRecord.get_txQ().multiply(
								new BigInteger(Long.toString(newRecord
										.getTimestamp()
										- oldRecord.getTimestamp()))));
			}
			...
			this.rxQ = newRecord.get_rxQ().multiply(
					new BigInteger(Long.toString(this.getDurationms())));
			this.txQ = newRecord.get_txQ().multiply(
					new BigInteger(Long.toString(this.getDurationms())));
		}
		// Copied values:
	...
	public static int[] produceRecords(RecordQueue outputQueue,
		...
		try {
			...
			for (int pn = 0; pn < pPaths.length; pn++) {
				...
				if (fdPaths != null) {
					...
					for (int x = 0; x < fdPaths.length; x++) {
						try {
							String linkTarget = Files.readSymbolicLink(
									Paths.get(fdPaths[x].toString()))
									.toString();
							if (linkTarget.startsWith("socket:[")) {
								socketId = linkTarget.split("\\[")[1]
										.split("\\]")[0];
								String[] parts = null;
								if ((parts = recordMap.get(socketId)) != null) {
									TcpConnectionStatRecord record = null;
									try {
										record = new TcpConnectionStatRecord(
												parts, pid);
									} catch (Exception e) {
									...
									if (record != null
											&& !outputQueue.put(producerName,
													record)) {
										LOG.error(
												"Failed to put TcpConnectionStatRecord into output queue {}  size: {}  maxSize: {}  producerName: {} ",
			...
			// System.err.println("Generated " + recordsGenerated +
			// " output records from " + processesChecked + " processes and " +
	...
	public String toString() {
		if (getPreviousTimestamp() == -1)
			return super.toString() + " tcp.connection.stats "
					+ longIpToString(localIp) + ":" + localPort + " "
					+ longIpToString(remoteIp) + ":" + remotePort + " txQ:"
					+ txQ + " rxQ:" + rxQ + " pid:" + pid;
		else
			return super.toString() + " tcp.connection.stats "
					+ longIpToString(localIp) + ":" + localPort + " "
					+ longIpToString(remoteIp) + ":" + remotePort + " txQ:"
					+ (txQ.doubleValue() / (double) getDurationms()) + " rxQ:"
					+ (rxQ.doubleValue() / (double) getDurationms()) + " pid:"
					+ pid;
