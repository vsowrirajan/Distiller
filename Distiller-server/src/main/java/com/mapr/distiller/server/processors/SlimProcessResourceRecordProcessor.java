...
public class SlimProcessResourceRecordProcessor implements
	...
	public DifferentialValueRecord diff(Record rec1, Record rec2, String metric)
		...
		if (rec1.getPreviousTimestamp() == -1l
			...
			throw new Exception(
					"SlimProcessResourceRecords can only be diff'd from non-raw SlimProcessResourceRecords");
		...
		if (oldRecord.getStartTime() != newRecord.getStartTime()
				|| oldRecord.getPid() != newRecord.getPid())
			throw new Exception(
		...
		case "ioCallRate":
			return new DifferentialValueRecord(
					oldRecord.getPreviousTimestamp(), oldRecord.getTimestamp(),
					newRecord.getPreviousTimestamp(), newRecord.getTimestamp(),
					getName(), metric, "double", newRecord.getIoCallRate()
							- oldRecord.getIoCallRate());
		...
		case "iowaitTicks":
			return new DifferentialValueRecord(
					oldRecord.getPreviousTimestamp(), oldRecord.getTimestamp(),
					newRecord.getPreviousTimestamp(), newRecord.getTimestamp(),
					getName(), metric, "BigInteger", newRecord.getIowaitTicks()
							.subtract(oldRecord.getIowaitTicks()));
		...
		case "rss":
			return new DifferentialValueRecord(
					oldRecord.getPreviousTimestamp(), oldRecord.getTimestamp(),
					newRecord.getPreviousTimestamp(), newRecord.getTimestamp(),
					getName(), metric, "BigInteger", newRecord.getRss()
							.subtract(oldRecord.getRss()));

		case "ioCalls":
			return new DifferentialValueRecord(
					oldRecord.getPreviousTimestamp(), oldRecord.getTimestamp(),
					newRecord.getPreviousTimestamp(), newRecord.getTimestamp(),
					getName(), metric, "BigInteger", newRecord.getIoCalls()
							.subtract(oldRecord.getIoCalls()));

		case "ioBytesRead":
			return new DifferentialValueRecord(
					oldRecord.getPreviousTimestamp(), oldRecord.getTimestamp(),
					newRecord.getPreviousTimestamp(), newRecord.getTimestamp(),
					getName(), metric, "BigInteger", newRecord.getIoBytesRead()
							.subtract(oldRecord.getIoBytesRead()));
	...
	public boolean isEqual(Record record, String metric, String thresholdValue)
		...
		case "startTime":
			return currentRecord.getStartTime() == Long
					.parseLong(thresholdValue);

		case "ioCallRate":
			return currentRecord.getIoCallRate() == Double
					.parseDouble(thresholdValue);

		case "ioBytesRead":
			return currentRecord.getIoBytesRead().equals(
					new BigInteger(thresholdValue));

		case "ioBytesWritten":
			return currentRecord.getIoBytesWritten().equals(
					new BigInteger(thresholdValue));

		default:
			throw new Exception("Metric " + metric
					+ " is not Thresholdable in SlimProcessResourceRecord for "
	...
	public boolean isAboveThreshold(Record record, String metric,
		...
		case "cpuUtilPct":
			if (currentRecord.getCpuUtilPct() == -1d)
				throw new Exception(
						"Can not compare raw SlimProcessResourceRecord to threshold");
			else
		...
		case "readIoByteRate":
			if (currentRecord.getReadIoByteRate() == -1d)
				...
				return currentRecord.getReadIoByteRate() > Double
						.parseDouble(thresholdValue);
		...
		case "writeIoByteRate":
			if (currentRecord.getWriteIoByteRate() == -1d)
				...
				return currentRecord.getWriteIoByteRate() > Double
						.parseDouble(thresholdValue);

		case "iowaitTicks":
			return currentRecord.getIowaitTicks().compareTo(
					new BigInteger(thresholdValue)) == 1;

		case "cpuUsageTicks":
			return currentRecord.getCpuUsageTicks().compareTo(
					new BigInteger(thresholdValue)) == 1;

		case "ioCalls":
			return currentRecord.getIoCalls().compareTo(
					new BigInteger(thresholdValue)) == 1;

		case "ioBytesRead":
			return currentRecord.getIoBytesRead().compareTo(
					new BigInteger(thresholdValue)) == 1;

		case "ioBytesWritten":
			return currentRecord.getIoBytesWritten().compareTo(
					new BigInteger(thresholdValue)) == 1;

		case "rss":
			return currentRecord.getRss().compareTo(
					new BigInteger(thresholdValue)) == 1;
