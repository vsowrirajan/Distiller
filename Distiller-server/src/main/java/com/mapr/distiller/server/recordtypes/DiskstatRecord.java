		...
		try {
			...
			while ( (line = proc_diskstats.readLine()) != null) {
				...
				if(f.exists()){	
					...
					try{
						DiskstatRecord record = new DiskstatRecord(now, parts, hw_sector_size);
						if(!outputQueue.put(producerName, record)){
							throw new Exception("Failed to put DiskstatRecord into output queue " + outputQueue.getQueueName() + 
