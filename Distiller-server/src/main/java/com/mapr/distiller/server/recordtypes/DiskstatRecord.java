...
public class DiskstatRecord extends Record {
	 ...
	 */
	public static boolean produceRecords(RecordQueue disk_system, String producerName){
		...
		try {
			...
			while ( (line = proc_diskstats.readLine()) != null) {
				...
				if(f.exists()){	
					...
					try{
						disk_system.put(producerName, new DiskstatRecord(now, parts, hw_sector_size));
					} catch (Exception e) {
