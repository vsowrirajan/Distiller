...
public class ThreadResourceRecord extends Record {
	 ...
	 * DERIVED VALUES
	 */
	private double cpuUtilPct, iowaitUtilPct, readCallRate, writeCallRate,
			readCharRate, writeCharRate, readByteRate, writeByteRate,
			cancelledWriteByteRate;
	...
	private BigInteger delayacct_blkio_ticks, guest_time, majflt, minflt,
			stime, utime;
	 ...
	 * CONSTRUCTORS
	 */
	public ThreadResourceRecord(ThreadResourceRecord rec1,
		...
		if (rec1.get_starttime() != rec2.get_starttime()
			...
			throw new Exception(
					"Differential ThreadResourceRecord can only be generated from input records from the same process");
		if (rec1.getCpuUtilPct() != -1d || rec2.getCpuUtilPct() != -1d
				|| rec1.getPreviousTimestamp() != -1l
				|| rec2.getPreviousTimestamp() != -1l)
			throw new Exception(
					"Differential ThreadResourceRecord can only be generated from raw ThreadResourceRecords");
		if (rec1.getTimestamp() == rec2.getTimestamp())
			throw new Exception(
					"Can not generate differential ThreadResourceRecord from input records with matching timestamp values");
	...
	public ThreadResourceRecord(String statPath, String ioPath, int ppid,
		...
		try {
			...
			try {
				...
				throw new Exception(
						"Failed to produce a ThreadResourceRecord due to an exception reading the stats file: "
								+ statPath, e);
			}
			if (br > 0 && br < statbs) {
				...
				throw new Exception(
						"Failed to produce a ThreadResourceRecord from stat file due to read response length, br:"
								+ br + " bs:" + statbs);
			}
			...
			if (parts.length < 42) { // Expect 44 values in
										...
										// dev.
				throw new Exception(
						"Failed to produce a ThreadResourceRecord due to unexpected format of stat file, found "
								+ parts.length + " fields");
			}
			...
			try {
				...
				throw new Exception(
						"Failed to produce a ThreadResourceRecord due to an exception reading the io file: "
								+ ioPath, e);
			}
			if (br > 0 && br < iobs) {
				line = new String(b.array());
				...
				throw new Exception(
						"Failed to produce a ThreadResourceRecord from io file due to read response length, br:"
								+ br + " bs:" + iobs);
			}
	...
	public static int[] produceRecord(RecordQueue outputQueue,
		...
		if (record != null && !outputQueue.put(producerName, record)) {
			ret[3] = 1;
			System.err
					.println("Failed to put ThreadResourceRecord into output queue "
	...
	public String toString() {
		return super.toString() + " thread.resources: " + pid + " " + comm
				+ " " + state + " " + ppid;
	}
