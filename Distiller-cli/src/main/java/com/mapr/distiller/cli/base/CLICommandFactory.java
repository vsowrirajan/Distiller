...
public class CLICommandFactory {
  ...
  private static final Logger LOG = LoggerFactory
      .getLogger(CLICommandFactory.class);
  ...
  /**
   * Provide method to get list of all registered commands Needs to be enhanced
   * to provide their usage as well
   * 
   * @return
   */
    ...
    for (CLICommand cliCommand : allCommands.values()) {
      ...
      allCommandsUsage
          .put(
              ...
              (null == cliCommand.getShortUsage()) ? "short description is not available"
                  : cliCommand.getShortUsage());
    }

    for (Map.Entry<String, String> commandUsage : allCommandsUsage.entrySet()) {
      char[] asChars = Arrays.copyOf(commandUsage.getKey().toCharArray(),
          maxLength);
  ...
  /**
   * Figure out which CLIInterface corresponds best to input provided method is
   * looking recursively if needed through all subcommands
   * 
   * @param input
   *          - ProcessedInput
   * @return CLIInterface
   ...
   */
  public CLIInterface getCLI(ProcessedInput input)
      throws CLIProcessingException {
    if (input == null) {
      return null;
    }

    CLICommand cliCommand = CLICommandRegistry.getInstance().getCLICommand(
        input.getCommandName());
    if (cliCommand == null) {
      /**
       * <MAPR_ERROR> Message:CLICommand is not found: <command>
       * Function:CLICommandFactory.getCLI() Meaning:The command does not exist.
       * Resolution:Check the command syntax and make sure the command is typed
       * correctly. </MAPR_ERROR>
       */
      LOG.error("CLICommand not found: " + input.getCommandName());
      return null;
    }

    CLICommand cliConcreteCommand = findConcreteCLICommand(cliCommand,
        input.getSubCommandNames());
    if (cliConcreteCommand == null) {
      return null;
    }

    Class<? extends CLIInterface> cliConcreteClass = cliConcreteCommand
        .getCommandClass();
    if (cliConcreteClass != null) {
      try {
        Constructor<? extends CLIInterface> ctor = cliConcreteClass
            .getConstructor(ProcessedInput.class, CLICommand.class);
        CLIInterface returnInterface = ctor.newInstance(input,
            cliConcreteCommand);
        return returnInterface;
      } catch (Exception e) {
        throw new CLIProcessingException(
            "Exception during CLIInterface instantiation", e);
      }
    }
    return null;
  }
   ...
   */
  private CLICommand findConcreteCLICommand(CLICommand cliCommand,
      List<String> subcommandNames) throws CLIProcessingException {
    if (cliCommand == null) {
      /**
       * <MAPR_ERROR> Message:CLICommand not found
       * Function:CLICommandFactory.findConcreteCLICommand() Meaning:The command
       * does not exist. Resolution:Check the command syntax and make sure the
       * command is typed correctly. </MAPR_ERROR>
       */
    ...
    if (childCLICommands != null && !subcommandNames.isEmpty()) {
      for (CLICommand childCommand : childCLICommands) {
        if (childCommand.getCommandName().equalsIgnoreCase(
            subcommandNames.get(0))) {
          // let's explore further
          CLICommand cliChildCommand = findConcreteCLICommand(childCommand,
              subcommandNames.subList(1, subcommandNames.size()));
