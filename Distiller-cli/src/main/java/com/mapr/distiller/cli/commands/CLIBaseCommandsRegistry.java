...
/**
 * Local registry to keep commands local to CLIFramework - so far we have only
 * one which is SSH based
 */
public class CLIBaseCommandsRegistry implements CLIRegistryInterface {

  public static final CLICommand sshCommand = new CLICommand("ssh",
      "ssh <script_name>", BaseSSHCommand.class, ExecutionTypeEnum.SSH, null,
      null).setUsageInVisible(true).setShortUsage("ssh <script_name>");
