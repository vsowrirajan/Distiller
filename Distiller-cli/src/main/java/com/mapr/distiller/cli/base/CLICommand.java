...
/**
 * Class to keep together all the information that each command was registered with
 * @author yufeldman
 *
 */
public class CLICommand {
	 ...
	 * so far the only usage of this one to get visibility on
	 * parent command name
	 */
	...
	public CLICommand(String commandName, 
		...
		this(commandName, commandUsage, commandClass, executionTypeEnum, null, subcommands);		
	}
	...
	public CLICommand(String commandName,
			...
			Class<? extends CLIInterface> commandClass,
			ExecutionTypeEnum executionTypeEnum, 
			Map<String,BaseInputParameter> params,
			CLICommand [] subcommands) {
	...
	/**
	 * Returns recursively usage of this and all subcommands
	 * NOT USED effectively anymore
	 * @return
	 */
	...
	/**
	 * Get all parent hierarchy command names
	 * used for usage construction
	 * @return
	 */
	...
	/**
	 * Construct usage only for a particular command w/o taking into consideration children
	 * @return
	 */
	...
	/**
	 * Construct usage only for a particular command with all children commands usage
	 * and parent command names
	 * @return
	 */
	...
	String getUsageFromParameters(int tabCount) {
		...
		for ( Map.Entry<String, BaseInputParameter> entry : parameters.entrySet()) {
			...
			if ( !isValueRequired ) {
				...
				if ( isBoundaryApplicable && (lowBoundary != null || upperBoundary != null)) {
					sb.append(" <");
					sb.append((lowBoundary != null ) ? " Low Boundary: " + lowBoundary : "");
					sb.append((upperBoundary != null ) ? ". Upper Boundary: " + upperBoundary : "");
					sb.append(">");
				}
	...
	/**
	 * Enum to identify whether execution of the command should be coming from the code, or just SSH command
	 * @author yufeldman
	 *
	 */
